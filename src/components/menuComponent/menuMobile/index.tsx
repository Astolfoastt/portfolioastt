  ...
  return (
    <nav className={style.container}>
      ...
      <div className={style.buttonModal}>
        {isOpenModal? (
          <img src={closeMenu} alt="Ícone de fechar menu" onClick={toggleModal} /> 
        ) : (
          <img className={style.iconMenu} src={menuIcon} alt="icone abertura de menu" onClick={toggleModal} />
        )}
        {isOpenModal && <ModalMenu toggleModal={toggleModal}  />} 
      </div>
      <img className={style.iconMode} src={modeIcon} alt="icone alteração de tema" />
    </nav>
  );
};
